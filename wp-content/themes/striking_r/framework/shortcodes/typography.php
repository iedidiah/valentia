...
function theme_shortcode_list($atts, $content = null, $code) {
	...
	return str_replace('<ul>', '<ul class="'.$style.$icon.$color.$position.'">', do_shortcode(trim($content)));
}
...
function theme_shortcode_milestone($atts) {
	...
	return '<div class="milestone'.$icon_class.$size_class.'">'.$icon.'<div class="milestone_number"'.$number_color_style.' data-speed="'.$speed.'" data-from="'.$numberfrom.'" data-to="'.$number.'">'.$numberfrom.'</div><div class="milestone_subject"'.$subject_color_style.'>'.$subject.'</div></div>';
}
