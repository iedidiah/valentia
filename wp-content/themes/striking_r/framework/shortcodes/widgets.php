...
function theme_shortcode_archives($atts){
	...
	if($dropdown === 'true'){
		...
		$output = '<div class="widget_archive_wrap"><ul>'.wp_get_archives(apply_filters('widget_archives_args', array('type' => 'monthly', 'show_post_count' => $count, 'echo'=>0))).'</ul></div>';
	}
...
HTML;
		...
		$cat_args['title_li'] = '';
		$output = '<div class="widget_categories_wrap"><ul>'.wp_list_categories(apply_filters('widget_categories_args', $cat_args)).'</ul></div>';
	}
