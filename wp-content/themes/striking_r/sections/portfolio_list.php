...
function theme_section_portfolio_list($options){
	...
	while($r->have_posts()) {
		...
		if (($i % $column) == 0 && $column != 1) {
			$output .= '<li class="portfolio_item'.$with_image.'" data-id="'.get_the_id().'" data-type="'.$type.'" data-cat="' . implode(',', $terms_slug) . '">';
		} else {
			$output .= '<li class="portfolio_item'.$with_image.'" data-id="'.get_the_id().'" data-type="'.$type.'" data-cat="' . implode(',', $terms_slug) . '">';
		}
		$i++;
		...
		if($desc == 'true' && $type !== 'html'){
			if($advancedesc == 'true'){
				...
				$output .= '<div class="portfolio_desc">' . do_shortcode(get_the_excerpt()) . '</div>';
			}else{
		...
		if($type == 'html'){
			$output .= '<div class="portfolio_html">' . apply_filters('the_content', get_the_content()) . '</div>';
		}
		...
		if($type == 'gallery'&&!empty($image_ids)){
			...
			foreach($image_ids as $image_id){
				...
				$output .= '<a class="lightbox" href="'.$image_src[0].'" title="'. $image_title .'" data-fancybox-group="'.(($group=='true')?$rel_group:'gallery-'.get_the_ID()).'"'.$image_lightbox_fittoview.'>gallery-'.get_the_ID().'</a>';
			}
			$output .= '</div>';
		}
		$output .= '</div>';
