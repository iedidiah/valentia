var shortcode = {
	resizeIframe:function(){
		...
		window.frameElement.height = window.document.documentElement.scrollHeight;
	},
	init:function(){
		...
		var select=jQuery('[name="sc_tabs_initialTab"]');
		for (i=1;i<=tabs_number;i++){
			select.append(jQuery("<option></option>").attr("value",i).text(i)); 
		}
		
		jQuery('[name="sc_tabs_number"]').live("change",function(){
		...
		//accordion
		var accordion_number = jQuery('[name="sc_accordion_number"]').val();
		...
		var select=jQuery('[name="sc_accordion_initialTab"]');
		var options = select.attr('options');
		for (i=1;i<=accordion_number;i++){
			 select.append(jQuery("<option></option>").attr("value",i).text(i)); 
		}
		jQuery('[name="sc_accordion_number"]').live("change",function(){
	...
	generate:function(){
		...
		case 'tabs':
			...
			var initialTab = shortcode.getVal('tabs','initialTab');
			if(type == ''){
				type = 'tabs';
			}
			if(history===true){
		...
		case 'accordion':
			...
			var initialTab = shortcode.getVal('accordion','initialTab');
			if(initialTab != 1){
				initialTab = ' initialTab="'+initialTab+'"';
			}else{
				initialTab = '';
		...
		case 'widget':
			...
			case 'contactform':
				...
				var bgColor = shortcode.getVal('widget','contactform','bgColor');
				var textColor = shortcode.getVal('widget','contactform','textColor');
				if(bgColor != ''){
					bgColor = ' bgColor="'+ bgColor +'"';
				}
				if(textColor != ''){
				...
				if(content != ""){
					return '\n[contactform'+email+bgColor+textColor+']\n'+content+'\n[/contactform]\n';
				}else{
					return '\n[contactform'+email+bgColor+textColor+']\n';
				}
				break;
		...
		case 'slideshow':
			...
			switch(sub_type){
				case 'nivo':
					...
					var boxCols = shortcode.getVal('slideshow','nivo','boxCols');
					var boxRows = shortcode.getVal('slideshow','nivo','boxRows');
					...
					var caption = shortcode.getVal('slideshow','nivo','caption');
					...
					//}else{
						return '[slideshow type="nivo"'+category+number+width+height+effect+slices+boxCols+boxRows+animSpeed+pauseTime+controlNav+pauseOnHover+caption+']';
					//}
					break;
			};
			break;
		...
		case 'portfolio':
			...
			var order = shortcode.getVal('portfolio','order');
			var orderby = shortcode.getVal('portfolio','orderby');
			...
			return '[portfolio'+column+height+nopaging+sortable+max+cat+ids+title+desc+advanceDesc+more+moreText+group+lightboxTitle+order+orderby+']';
			break;
		case 'blog':
			var column = shortcode.getVal('blog','column');
			var grid = shortcode.getVal('blog','grid');
			var count = shortcode.getVal('blog','count');
			var posts = shortcode.getVal('blog','posts');
			var cat = shortcode.getVal('blog','cat');
			var author = shortcode.getVal('blog','author');
			...
			return '[blog'+column+grid+count+posts+cat+author+image+width+height+meta+desc+full+nopaging+']';
			break;
		case 'sitemap':
			...
			switch(sub_type){
				...
				case 'posts':
					...
					var cat = shortcode.getVal('sitemap','posts','author');
					...
					return '[sitemap type="posts"'+show_comment+number+posts+cat+author+']';
					break;
				case 'portfolios':
	...
	sendToEditor :function(){
		...
		win.send_to_editor(shortcode.generate());
	},
