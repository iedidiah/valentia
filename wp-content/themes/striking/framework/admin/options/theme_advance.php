...
if (! function_exists("theme_advance_reset_options_process")) {
	function theme_advance_reset_options_process($option,$data) {
		if($data == true){
			...
			foreach($theme_options as $page=>$options){
				unset($theme_options[$page]);
	...
	*/
	...
	array(
		...
		array(
			...
			"desc" =>  __("Fix 'Disable Breadcrumbs' option on Page General Module issue after updating < version 3.0.1 to the new one. Do not try this if it's a new installation. You only need to enable this once."),
			"default" => false,
