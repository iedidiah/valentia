...
<object{$id} classs="flash" width="{$width}" height="{$height}" type="application/x-shockwave-flash" data="{$src}">
...
function theme_video_youtube($atts, $content=null) {
	...
	if (!$enablejsapi){
		$enablejsapi = (theme_get_option('video','youtube_enablejsapi') == true)?1:0;
	}else{
		if($enablejsapi==='true'){
			$enablejsapi = '1';
		}else{
	...
	if (!empty($clip_id)){
		echo "<div class='video_frame'><iframe class='youtube' style='height:{$height}px;width:{$width}px' src='http://www.youtube.com/embed/{$clip_id}?autohide={$autohide}{$autoplay}&amp;controls={$controls}&amp;disablekb={$disablekb}&amp;fs={$fs}&amp;hd={$hd}&amp;loop={$loop}&amp;rel={$rel}&amp;showinfo={$showinfo}&amp;showsearch={$showsearch}&amp;wmode=transparent&amp;enablejsapi={$enablejsapi}' width='{$width}' height='{$height}' frameborder='0'></iframe></div>";
	}
}
