	...
	function introduce($post_id = NULL) {
		...
		if (is_single() || is_page() || (is_front_page() && $post_id != NULL) || (is_home() && $post_id != NULL)){
			...
			if ($type == 'slideshow'){
				...
				return theme_generator('slideShow',$stype,$scategory,$color);
			}
...
<?php 
	...
	function slideShow($type, $category = '', $color = '') {
		/** fix **/
		if(empty($category)){
			$category = 's|all'; 
	...
	function slideShow_nivo($category='',$color='') {
		if(!empty($color) && $color != "transparent"){
			$color = ' style="background-color:'.$color.'"';
		}else{
			$color = '';
...
HTML;
		...
		foreach($images as $image) {
			...
			if($image['link'] != ''){
				echo '<a href="'.$image['link'].'" target="'.$image['target'].'"><img src="' . THEME_INCLUDES.'/timthumb.php?src='.get_image_src($image['src']).'&amp;h='.$height.'&amp;w=960&amp;zc=1' . '" title="'.$title.'" alt="" /></a>';
			}else{
				echo '<img src="' . THEME_INCLUDES.'/timthumb.php?src='.get_image_src($image['src']).'&amp;h='.$height.'&amp;w=960&amp;zc=1' . '" title="'.$title.'" alt="" />';
			}
...
HTML;
	...
	function slideShow_3d($category='',$color='') {
		if(!empty($color) && $color != "transparent"){
			$color = ' style="background-color:'.$color.'"';
		}else{
			$color = '';
...
HTML;
	...
	function slideShow_kwicks($category='',$color='') {
		if(!empty($color) && $color != "transparent"){
			$color = ' style="background-color:'.$color.'"';
		}else{
			$color = '';
...
HTML;
	...
	function slideShow_anything($category='',$color) {
		if(!empty($color) && $color != "transparent"){
			$color = ' style="background-color:'.$color.'"';
		}else{
			$color = '';
